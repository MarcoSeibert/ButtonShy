...
def extract_images(args):
    ...
    for page_num in range(len(doc)):
        ...
        for img_index, img in enumerate(page.get_images(full=True)):
            ...
            with open(f"{output_folder}/{game_name}_{page_num}_{img_index}.png", "wb") as f:
                f.write(image_bytes)
...
def show_loading_window():
        ...
        while processed_games < progress["maximum"]:
            try:
                ...
                if game_name not in processed_names:
                    ...
                    avg_time_per_game = total_elapsed / len(processed_names) if processed_names else 5.0
    ...
    Thread(
        target=check_for_assets,
        args=(result_queue, games_to_process),
        daemon=True
    ).start()
